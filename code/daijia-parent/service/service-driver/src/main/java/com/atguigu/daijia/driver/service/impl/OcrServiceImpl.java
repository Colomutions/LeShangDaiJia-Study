...
public class OcrServiceImpl implements OcrService {
    ...
    public IdCardOcrVo idCardOcr(MultipartFile file) throws Exception {
            ...
            String base64File = new String(base64);
            ...
            req.setImageBase64(base64File);
            IDCardOCRResponse resp = client.IDCardOCR(req);
            ModelUtils.nonNull(resp, "识别异常，身份证可能为假");
            String jsonString = JSON.toJSONString(resp);
            log.info("身份证识别结果为：{}", jsonString);
            IdCardOcrVo idCardOcrVo = new IdCardOcrVo();
            if (StringUtils.hasText(resp.getName())) {
                log.info("此处能够识别到姓名，证明是正面");
                ...
                log.info("此处无法识别到姓名，证明是反面");
                idCardOcrVo.setIdcardExpire(DateTimeFormat.forPattern("yyyy.MM.dd").parseDateTime(resp.getValidDate().split("-")[1]).toDate());
                //上传身份证反面图片到腾讯云cos
                CosUploadVo cosUploadVo = cosService.upload(file, "idCard");
